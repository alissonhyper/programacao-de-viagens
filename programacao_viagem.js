...
// Preenche a data com hoje, se estiver vazia
if (!campoData.value) {
    campoData.value = hoje;
}

// Define horário padrão (07:00) se estiver vazio
if (!campoInicio.value) {
    campoInicio.value = "07:00";
}
...
function gerarEncerramento() {
    ...
    t += `FECHAMENTO DA VIAGEM - ${formatarData(data)}\n`;
    t += "----------------------------------------------------------------------------\n";
    t += `🔧 EQUIPE: ${tecnico} e ${auxiliar}\n`;
    t += `📍 DESIGNAÇÃO: ${listaComE(cidades)} – ${listaComE(servicos)}\n`;
    const inicio = document.getElementById("inicio").value;
...
// =======================================
// CONTADOR DE SERVIÇOS MARCADOS (ACCORDION)
// =======================================
function atualizarQuantidadeServicos() {
    const servicosMarcados = document.querySelectorAll('.servico:checked').length;
    const titulo = document.getElementById('titulo-servicos');

    if (titulo) {
        titulo.textContent = `Serviços agendados (${servicosMarcados})`;
    }
}

// Atualiza ao carregar a página (ex: dados do localStorage)
document.addEventListener("DOMContentLoaded", function () {
    atualizarQuantidadeServicos();
